...
| The drift brake: hold Space and the engine cuts (the throttle setting is kept for the release), the retros bleed speed at full engine thrust, drag stays at the thrusting value so the momentum carries, and the nose swings free of it, to bring the gun round on a raider going past; the THRUST gauge reads DRIFT. Space no longer fires the weapon (LMB or L do) | `Assets/Scripts/Ship.cs` (`drifting`) | new in the Unity port |
