...
| The drift brake: hold Space and the engine cuts (the throttle setting is kept for the release), the retros bleed speed at ten times engine thrust (a stop from base top speed in 0.15 s, from a ×5 burner in 0.8 s), drag stays at the thrusting value so the momentum carries, and the nose swings free of it at 60° a second (twice the usual rate), to bring the gun round on a raider going past; the THRUST gauge reads DRIFT. Space no longer fires the weapon (LMB or L do) | `Assets/Scripts/Ship.cs` (`drifting`) | new in the Unity port |
...
| Coasting: the zero-throttle drag is 0.064/s (the browser's 1.28 cut twenty-fold), so with the throttle cut the ship all but keeps its way, about 3,900 m from top speed; the S retros and the drift brake are the stopping | `Assets/Scripts/Ship.cs` (`dragK`) | changed from the browser |
