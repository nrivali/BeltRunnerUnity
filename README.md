...
| The autocannon's shot is the user's own clip (`blaster.mp3`), played through `Audio.Shot`: a dedicated source restarted on every shot (the pool would queue or skip at six a second) with 200 ms cut off the clip's head at load (the recording's build-up) and any silence after that trimmed, so the sound lands on the shot; the raiders' shots keep the `zap`. |
