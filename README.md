...
| Combat music: a ninth track, Vanguard (156 bpm, minor, a kick on every beat, a driving sawtooth bass, square arps, stabs and a lead over a strings pad), that the music engine cuts to on the step raiders attack (the groove straight on, a quarter-second glide) and leaves six seconds after the all-clear, on a bar, drifting back to the track it was on; a toast names it | `Assets/Scripts/Music.cs` (`COMBAT`, `combat`, the fight block in `Schedule`) | new in the Unity port |
