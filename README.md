...
| Camera feel: the field of view (62°) opens on the afterburner (to 72° plus 2° a refit level, 74° at ×2), closes to 53° on the drift brake and 57° on the retros, and eases out a few degrees with speed; on a turn the chase camera hangs back on the outside, the look point leads into it, and the frame banks up to 7° with the yaw (4° with roll), all eased at 5/s so it settles rather than snaps | `Assets/Scripts/Ship.cs` (`UpdateCamera`, `FOV`) | new in the Unity port |
