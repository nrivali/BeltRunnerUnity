...
| The hangar window (replaces the two side panels): one centred glass window with a tab bar. Inventory: the hold's grid on the left and the cargo ship storage's on the right (drag between them or double-click; in flight the hold alone, with jettison), the storage / fuel / parts gauges below, and at the Hub the market. Ship refits and Cargo ship refits: the upgrade rows. Docking opens it on Inventory; Tab or I opens it anywhere on Inventory (only that tab in flight); F opens it on Ship refits while docked (and anywhere in the combat test, F8); Esc or Close shuts it; the footer keeps Depart / Nav map, Warp to the Hub and Reset save | `Assets/Scripts/Hud.cs` (`BuildWindow`, `RefreshWindow`, `InventoryTab`, `ShipTab`, `DepotTab`) | new layout, 2026-09-14 |
