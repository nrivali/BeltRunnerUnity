...
| Volumetric dust (2026-09-15): a ray march (`BeltRunner/Volumetric`) at half resolution from the camera along each pixel's view ray, 48 dithered steps out to 6,000 u or the depth buffer, through thin dust (0.00005 per unit, drifting value-noise clumps) that scatters the sun toward the eye (Henyey-Greenstein, g 0.85, gain 0.5: tight round the sun) and is shadowed by the sun's own shadow map, copied to `_BeltCascadeShadow` by a command buffer after the sun draws it and sampled through Unity's single-cascade matrix and bounds, so a rock between the sun and the dust cuts a dark shaft; added in the post composite. V toggles it (the toast says which) | `Assets/Scripts/Lighting.cs` (`Post.Dust`), `Assets/Resources/Shaders/Volumetric.shader`, `Post.shader` | new |
