...
| The deep-space backdrop (2026-09-15): the sky shader draws faint blue-grey wisps of nebulosity across the whole sky (the coloured clouds down to a trace), and a dense star field after the user's star-field reference: a fine dust of small stars (a gaussian a pixel or two wide, so none fall between pixels), a medium layer, and a few bright ones with four-point diffraction spikes; all subtle (the clouds at 0.3 to 0.4; a galaxy band was tried and dropped as too much) and under the bloom threshold, so space still reads black and the sun stays the brightest thing | `Assets/Resources/Shaders/Sky.shader` | new |
