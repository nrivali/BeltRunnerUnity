...
| The autocannon, fitted from the start (8 damage at 4 shots a second, 900 m) with three refit levels up to 26 damage at 8 a second and 1,700 m (900, 3,200 and 9,000 cr), fired with the laser trigger when a raider is under the nose (within 4°) and no rock is in the way, or at a locked raider anywhere across the forward half (the cannon rides the dish turret, which swings onto it); the bolts lead the target | `Assets/Scripts/Data.cs` (`gun`), `Assets/Scripts/Ship.cs` (`TickLaser`) | the ship always has it |
