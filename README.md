...
| The afterburner, the whole of it: in the post pass a radial blur toward a centre that leads into the turn (ten taps, 4.5% to 10.5% of the ray with distance out), the red and blue fringing apart along the same rays, and the edges pulled darker by up to 42%, all scaled by an intensity that eases in over a third of a second and out over half and grows a little with the refit level (`_Burn`, `_BurnCenter` in `Post.shader`, driven from the HUD); in the world the exhaust glows go white-blue at 30 u and flicker hard, and a plume of pale jet puffs streams off every engine at 40 a second (`Explosions.Jet`); the field of view kicks a further 7° at ignition and settles; the camera carries a low rumble, harder with the bigger refits; the streaks below stay under it, faint | `Post.shader`, `Lighting.cs` (`Post.burn`), `Ship.cs` (`TickEngineFx`, `UpdateCamera`, `burnK`), `Hud.cs` | new in the Unity port |
