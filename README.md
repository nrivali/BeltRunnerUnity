...
| Speed lines on the afterburner: 56 faint pale streaks fixed in space in a tube round the line of flight (45 to 190 u off it, up to 1,400 u ahead), each a soft quad stretched along the flight (30 to 160 u with speed) and turned to the camera, so the ship streams past them; they fade in over about a third of a second when the burner lights and out over half a second when it stops, and one that falls 220 u behind respawns ahead | `Assets/Scripts/SpeedLines.cs` | new in the Unity port |
