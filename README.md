...
| The raider boost: on a run in from beyond 900 u or a long run out, with the nose within about 37° of where it wants to go, the throttle goes wide open: a quarter more speed (`BOOST_MULT`), 1.6× the acceleration, the exhaust flaring white-blue at twice its size, a stream of pale jet puffs behind it (`Explosions.Jet`), and an ElevenLabs whoosh-and-roar (`raider_boost`) with distance the moment it kicks in | `Raiders.cs` (`SetBoost`) | new in the Unity port |
