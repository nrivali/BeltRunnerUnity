...
| The autocannon, fitted from the start (8 damage at 4 shots a second, 900 m) with three refit levels up to 26 damage at 8 a second and 1,700 m (900, 3,200 and 9,000 cr), selected with the scroll wheel (which swaps between the laser and the cannon; the WEAPON readout shows which) and fired with the trigger straight down the nose, no auto-aim: a crosshair marks the point the nose ray reaches at gun range, and with a raider locked (or under the nose) an amber LEAD pip marks where a bolt fired now would meet it, so the pilot flies the crosshair onto the pip. A raider lock only tracks; the ship steers itself onto rocks and the cargo ship only | `Assets/Scripts/Data.cs` (`gun`), `Assets/Scripts/Ship.cs` (`TickLaser`) | the ship always has it |
