...
| The autocannon, fitted from the start (8 damage at 6 shots a second, 900 m; a bolt hits within 60 u of a raider, and a hit marker, four white strokes, flashes over the crosshair when one lands, red for the kill, with a punchy tick (`hit_marker`, ElevenLabs), peak-normalized at load and played through `Audio.Sure`, a source of its own, since the eight-voice pool fills up in a fight and dropped it; the kill has only the boom) with three refit levels up to 26 damage at 10 a second and 1,700 m (900, 3,200 and 9,000 cr), selected with the scroll wheel (which swaps between the laser and the cannon; the WEAPON readout shows which) and fired with the trigger at the crosshair, which is the mouse: bolts leave the dish for the point under the cursor at gun range (held to the forward half, the arc the dish turret covers), no auto-aim; with a raider locked (or under the crosshair) an amber LEAD pip marks where to put the crosshair so a bolt fired now meets it (the intercept point, run back through the aim mapping: the dish sits off the camera, so a target nearer than gun range is not where it appears on screen). Locking a raider also has the ship follow it: the nose stays on the raider while the mouse works the gun. While raiders are attacking (`Ship.InCombat`) only raiders can be hovered or locked, and a rock or cargo ship lock is dropped with a toast | `Assets/Scripts/Data.cs` (`gun`), `Assets/Scripts/Ship.cs` (`TickLaser`) | the ship always has it |
