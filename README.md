...
| Menu sounds: a soft tick under the mouse on any button or link, a click on a press, a two-tone blip on a tab or menu page change, and a glassy swoosh as the hangar window opens and closes (five ElevenLabs clips, `ui_hover/click/tab/open/close`, hooked in `Ui.Face`, `Ui.Button`, `Ui.Link`, `Hud.ShowWindow/CloseWindow/PickTab`, `Menu.ShowPage`). The window keeps its scroll position when a purchase rebuilds it. In the game's text the refits are called upgrades now |  |  |
