...
| Vega's fuel calls: at 50%, 25% and 5% of the tank she speaks up once each on the way down (three ElevenLabs lines, `vega_fuel50/25/5`, calm and clear, from `tools/gen-voice.ps1`'s second list; `vega_` clips play clean like the tutorial, no squelch), with a toast; a refill above a mark re-arms it, and nothing is said while docked or during a recovery | `Assets/Scripts/Ship.cs` (`TickFuelWarnings`) | new in the Unity port |
