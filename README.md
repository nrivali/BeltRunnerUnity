...
jumps to the next raider hold, so a fight can be re-run without flying back. In the test the raiders of the hold hold their place (they still turn to face the ship and fire), one extra raider that moves is added beside them, and a raider killed comes back where it stood three seconds on; all of this is temporary while the gun is tuned.
...
| Piece | Where | Notes |
|---|---|---|
| Raider holds: a hold of one to three raiders at some of the rich pockets (three holds, plus six per point of zone danger; Kessler's danger is 0.5, so six holds), wandering round home. Within 6,500 u of a flying ship outside the cargo ship's gun cover they attack: close to 900 u, orbit, and fire bolts that lead the ship whenever their nose is within 20° of it and it is within 2,500 m, since the guns are fixed forward (a bolt every 0.5 s at 2,200 u/s, 5 damage in Kessler, poor aim in a quiet zone). Each carries a 50-point shield that soaks damage first and recharges (10 a second) after ten seconds without a hit, over 50 health. They fly a weaving run in, a strafing pass at a radius (300 to 700 u) and direction of their own, then a breakaway and round again, or a long run out to a point 2,000 to 5,000 m from the ship before coming back in, and jink aside when a bolt is coming their way (seven times in ten, once every 1.6 s at most). They fly like ships: the nose turns at 35° a second at most, thrust and braking are limited, and they only ever move along the nose, so nothing they do is a snap the player could not match. They give up beyond 11,000 u, or when the ship is disabled or docked | `Assets/Scripts/Raiders.cs` | primitives for the hull (cone body, swept wing, red trim, an eye, an exhaust glow); health 50 and shield 50, speed 865, bounty 140 cr at danger 0.5 |
...
| The autocannon, fitted from the start (8 damage at 4 shots a second, 900 m) with three refit levels up to 26 damage at 8 a second and 1,700 m (900, 3,200 and 9,000 cr), selected with the scroll wheel (which swaps between the laser and the cannon; the WEAPON readout shows which) and fired with the trigger: at the raider under the nose (within 4°) or a locked one anywhere across the forward half (the cannon rides the dish turret, which swings onto it), the bolts leading the target, or straight down the nose with nothing in the sights | `Assets/Scripts/Data.cs` (`gun`), `Assets/Scripts/Ship.cs` (`TickLaser`) | the ship always has it |
