...
| Detail on big rocks (2026-09-15): the rock shader's chipped-stone detail layer tiles by the rock's radius (once per 22 u) and firms up a little with size, so a big rock keeps the same grain per metre up close as a small one instead of stretching its maps | `Assets/Resources/Shaders/Rock.shader` | new |
...
| The autocannon, fitted from the start (8 damage at 6 shots a second, 5,000 m at every level; a bolt hits within 60 u of a raider, and a hit marker, four white strokes, flashes over the crosshair when one lands, red for the kill, with a punchy tick (`hit_marker`, ElevenLabs) and a sting on the kill (`kill_marker`), both peak-normalized at load and played through `Audio.Sure`, a source of their own, since the eight-voice pool fills up in a fight and dropped them) with three refit levels up to 26 damage at 10 a second (900, 3,200 and 9,000 cr), selected with the scroll wheel (which swaps between the laser and the cannon; the WEAPON readout shows which) and fired with the trigger straight down the nose, like the laser (the mouse steers the ship, so the pilot flies the nose onto the target), no auto-aim; every raider within gun reach carries an amber LEAD pip marking where to put the crosshair so a bolt fired now meets it, and raiders carry no reticle (the intercept point, run back through the aim mapping: the dish sits off the camera, so a target nearer than gun range is not where it appears on screen). Locking a raider also has the ship follow it: the nose is steered onto its LEAD point, where a bolt fired now would meet it, so a locked raider is hit by holding the trigger. While raiders are attacking (`Ship.InCombat`) only raiders can be hovered or locked, and a rock or cargo ship lock is dropped with a toast. Raiders are easy to pick for Q: a circle of at least 150 px round each (or 2.5 times its drawn size), the one nearest the mouse wins, and a raider beats any rock under the mouse | `Assets/Scripts/Data.cs` (`gun`), `Assets/Scripts/Ship.cs` (`TickLaser`) | the ship always has it |
