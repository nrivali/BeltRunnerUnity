...
| Not every kill is a blast: 45% of the time (`Raiders.BURN_CHANCE`) the raider catches fire and loses control instead, flying on with the momentum it had while a tumble builds (to 220°/s), trailing flame puffs, smoke and sparks with the exhaust guttering, for 3.5 to 8 s, and then it goes up in the full explosion, the hull coming apart, wherever that leaves it; the hit that did it is a metallic thud with distance | `Raiders.cs` (`StartBurn`, `TickHulks`), `Explosions.Flame`, `Explosions.Smoke` | new in the Unity port |
