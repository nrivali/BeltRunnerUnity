...
| The belt seen from afar (2026-09-15): a ring volume round the planet (`BeltRunner/Ring`), the inside faces of a box round the whole belt, a child of the planet; each pixel intersects its view ray with the annular slab (the three base belts by radius, each a gaussian across its spread, the whole 1.6 × the widest half-spread thick), clips it to the depth buffer, and marches 20 dithered samples of procedural density (streaks along the orbit and grain in polar coordinates, 0.0000038 per unit), lit by the sun with a little back-scatter and dark in the planet's shadow, fading in past half the rocks' draw distance so the drawn rocks are never doubled. From inside the belt it is the band across the sky all the way round; from above, the ring | `Assets/Scripts/Game.cs` (`BuildRings`), `Assets/Resources/Shaders/Ring.shader` | new |
