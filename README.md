...
| The HUD regrouped (2026-09-15): one band of three glass panes along the bottom, 920 by 100, in the same style as before. SHIP: hull, shield, fuel and the hold, two by two. FLIGHT: the zone in its eyebrow, the speed with its unit and the thrust gauge, then the cargo ship's distance and the field, the radar and the threat. TARGET: what the crosshair or the lock is on (name, size, range, health, the warning), the weapon in hand with the laser's state, or "No target"; hidden while docked. The top-right readouts and the floating target card are folded into it, the controls list sits above the band, and there is no radar display | `Assets/Scripts/Hud.cs` (`BandPane`, `BuildStatus`, `BuildReadouts`, `BuildTarget`) | new layout |
