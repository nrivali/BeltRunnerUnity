...
| Six ends for a raider (`Raiders.BLAST_CHANCE` 30% for the first; the rest split): the blast on the spot; the burn (catches fire and tumbles, flame, smoke and sparks, the exhaust guttering, 3.5 to 8 s); the chain (a run of pops along the hull every 0.08 to 0.16 s for about a second); the runaway (the engine jams open, blazing white-blue, and the hulk accelerates at 420 u/s² along a wandering nose, corkscrewing away with a torch of flame and thick smoke, for 1.5 to 3 s); the shed (pieces tear off every 0.3 to 0.6 s with a pop, a puff and sparks, until only the body is left); the dead hull (lights out, a steady flat spin for 10 to 18 s with blue-white arcs crawling over it and crackling). Only the blast on the spot explodes: every wreck ends quietly, coming apart with a last gout of flame and smoke (arcs for the dead hull) and no blast, so a raider blows up 30% of the time. Every wreck flies on with the momentum it had; the hit that did it is a metallic thud with distance | `Raiders.cs` (`StartWreck`, `TickHulks`, `QuietBreak`), `Explosions.Flame`/`Smoke`/`Pop`/`Arc` | new in the Unity port |
