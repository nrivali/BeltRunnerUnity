...
| The autocannon's shot is the user's own clip (`blaster.mp3`), played through `Audio.Shot`: a dedicated source restarted on every shot (the pool would queue or skip at six a second) with the clip's leading encoder silence trimmed off at load, so the sound lands on the shot; the raiders' shots keep the `zap`. |
