...
public class Menu
{
    ...
    static readonly object[][] CONTROLS =
    {
        ...
        new object[] { new[] { "Z", "MMB" }, "Hover the mouse over a rock or the cargo ship (the label names it), then press Z or the middle mouse button to lock the crosshair on it. The ship steers itself to keep it in the crosshair (you keep the throttle and roll) until it breaks up or goes beyond " + Data.Fm(Ship.LOCK_RANGE) + " m. Hover a different target and press Q to switch directly to it. Press Q over the current target or empty space to release the lock. A locked object always shows its range, and its details sit top centre." },
