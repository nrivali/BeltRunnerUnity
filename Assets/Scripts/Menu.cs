...
public class Menu
{
    ...
    static readonly object[][] CONTROLS =
    {
        ...
        new object[] { "Raiders", "Pirate raiders hold station off the rich pockets (the KP fields) and attack within 3,250 m. Press 2 (or roll the scroll wheel) to select the autocannon, 1 for the mining laser (fitted from the start; the refits sharpen it) and hold LMB: bolts go straight down the nose, where the crosshair is (the mouse steers the ship, so you fly the nose onto the target). Q locks a raider: the ship steers its nose onto the raider's LEAD pip, where a bolt fired now would meet it, so holding the trigger hits; every raider within gun reach carries a pip, and the locked one wears the amber brackets. While raiders are attacking nothing else can be locked or picked, and a rock or cargo ship lock is dropped. The cargo ship's guns cover 4,500 m round it. Lose the hull to them and they strip a third of your hold before recovery." },
