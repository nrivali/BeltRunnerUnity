...
public class Menu
{
    ...
    static readonly object[][] CONTROLS =
    {
        ...
        new object[] { new[] { "P" }, "Cockpit view on · off: the camera at the pilot's eye, rigid to the hull, a wider lens, the canopy frame and the dash round the view; C or the right button still turns your head. Remembered in the save" },
        ...
        new object[] { new[] { "Z", "MMB" }, "Hover the mouse over a rock or the cargo ship (the label names it), then press Z or the middle mouse button to lock it. A locked rock is bracketed and its range shown, and the laser is still yours to aim: put the crosshair on the rock. A locked cargo ship or raider steers the ship itself to keep it in the crosshair (you keep the throttle and roll). A lock lapses when its rock breaks up or it goes beyond " + Data.Fm(Ship.LOCK_RANGE) + " m. Hover a different target and press Q to switch directly to it. Press Q over the current target or empty space to release the lock. A locked object always shows its range, and its details sit top centre." },
        ...
        new object[] { "Raiders", "Pirate raiders hold station off the rich pockets (the KP fields) and attack within " + Data.Fm(Raiders.ENGAGE) + " m. Press 2 (or roll the scroll wheel) to select the autocannon, 1 for the mining laser (fitted from the start; the refits sharpen it) and hold LMB: bolts go straight down the nose, where the crosshair is (the mouse steers the ship, so you fly the nose onto the target). Q locks a raider: the ship steers its nose onto the raider's LEAD pip, where a bolt fired now would meet it, so holding the trigger hits; every raider within gun reach carries a pip, and the locked one wears the amber brackets. While raiders are attacking nothing else can be locked or picked, and a rock or cargo ship lock is dropped. The cargo ship's guns cover " + Data.Fm(Raiders.SAFE_R) + " m round it. Lose the hull to them and they strip a third of your hold before recovery. The raider outpost is harder: its turrets reach " + Data.Fm(Outpost.TURRET_REACH) + " m with a faster bolt, and its battery fires seeker rockets at you. B drops a flare, a burning decoy the rockets go for instead (" + Ship.FLARES + " aboard, restocked on the pad); the autocannon can shoot a rocket down too, and a hard turn at the last moment can throw one." },
        new object[] { "Docking", "Fly slowly into either mouth of the cargo ship's through-hangar, or press E within " + Data.Fm(Data.DOCK_RANGE) + " m of it and approach control flies you in. Once docked, E deposits all your ore into the cargo ship's storage. Fuel and repairs flow while you sit on the pad. The cargo ship warps with you (H on the pad jumps to the Hub); ore only sells at the Hub." },
        ...
        new object[] { new[] { "F9" }, "Testing: clear the zone's raiders and spawn three fresh ones 500 to 1,000 m out in random directions, mostly ahead, homed on you so they attack at once. Launching the game with -combat starts that way in a sandbox that never touches the save." },
