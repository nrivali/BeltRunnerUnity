...
public class Menu
{
    ...
    static readonly object[][] CONTROLS =
    {
        ...
        new object[] { "Raiders", "Pirate raiders hold station off the rich pockets (the KP fields) and attack within 3,250 m. Roll the scroll wheel to select the autocannon (fitted from the start; the refits sharpen it) and hold LMB: bolts go from the dish to the crosshair, which is the mouse (the dish turret covers the forward half, so nothing fires behind you). Q locks a raider: the ship follows it, keeping its nose on the raider while you work the gun with the mouse, and an amber LEAD pip marks where to put the crosshair so a bolt fired now meets it. While raiders are attacking nothing else can be locked or picked, and a rock or cargo ship lock is dropped. The cargo ship's guns cover 4,500 m round it. Lose the hull to them and they strip a third of your hold before recovery." },
